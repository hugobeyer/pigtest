...
from config import CAMERA_RESOLUTION, CAMERA_DISTANCE, CAMERA_ELEVATION_DEGREES, CAMERA_OFFSET_Y, CAMERA_OFFSET_Z, CAMERA_LENS
...
def build_camera(collection, target):
  ...
  if camera:
    if camera.data.type=='ORTHO':
      camera.data.lens=camera.data.sensor_width*(camera.location-target.location).length/camera.data.ortho_scale
